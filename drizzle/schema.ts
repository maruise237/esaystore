...
export const movementTypeEnum = pgEnum("stock_movement_type", ["opening", "restock", "adjustment", "sale", "return"]);
export const paymentMethodEnum = pgEnum("payment_method", ["cash", "mobile_money", "credit", "mixed"]);
export const saleStatusEnum = pgEnum("sale_status", ["completed", "cancelled"]);
export const syncKindEnum = pgEnum("sync_kind", ["sale", "expense", "repayment", "adjustment"]);
...
export const products = pgTable("products", {
  ...
  category: varchar("category", { length: 120 }).default("Sans catégorie").notNull(),
...
export const sales = pgTable("sales", {
  ...
  uniqueIndex("sales_shop_operation_unique").on(table.shopId, table.operationId),
...
export const stockMovements = pgTable("stock_movements", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("stock_movements_shop_created_idx").on(table.shopId, table.createdAt)]);
...
export const receivables = pgTable("receivables", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [index("receivables_shop_customer_idx").on(table.shopId, table.customerId)]);
...
export const repayments = pgTable("repayments", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [uniqueIndex("repayments_shop_operation_unique").on(table.shopId, table.operationId)]);
...
}, (table) => [uniqueIndex("sync_operations_shop_operation_unique").on(table.shopId, table.operationId)]);
