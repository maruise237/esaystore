...
function Reports({ shopId, currency }: { shopId: string; currency: string }) {
  ...
  const report = trpc.insights.report.useQuery({
    shopId,
    ...
    <div className="space-y-6">
      <Card className="border-0 bg-[#edf1e3]">
        <CardContent className="flex flex-wrap items-end gap-4 p-5">
          <div>
            <p className="font-serif text-xl">Période analysée</p>
            <p className="mt-1 text-xs text-[#85877f]">
              Changez les dates pour actualiser les indicateurs.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <ReportMetric
          label="Marge brute"
          value={currencyFormat(data?.grossMargin ?? 0, currency)}
        />
        <ReportMetric
        ...
        <CardContent className="p-6">
          ...
          <div className="mt-5 space-y-3">
            {data?.topProducts.map((item, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <span className="grid h-7 w-7 place-items-center rounded-full bg-[#dbe8bc] text-xs font-bold">
                    {index + 1}
                  </span>
                  <div>
