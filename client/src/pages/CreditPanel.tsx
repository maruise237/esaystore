...
import { Download, Loader2, UserRoundPlus, WalletCards } from "lucide-react";
...
export default function CreditPanel({
  ...
  const recordRepayment = async (receivableId: string, balance: number) => {
    const amount = Number(repayments[receivableId]);
    ...
    if (!navigator.onLine) {
      await queueOperation("repayment", payload);
      setRepayments(values => ({ ...values, [receivableId]: "" }));
      return;
    }
    repay.mutate(payload, {
      onSuccess: () =>
        setRepayments(values => ({ ...values, [receivableId]: "" })),
    });
  };
  const exportReceivables = () => {
    ...
    downloadCsv(
      `easystor-creances-${status}${overdueOnly ? "-echues" : ""}.csv`,
      createCsv(
        [
          "Client",
          "Créée le",
        ...
        receivables.data.map(({ receivable, customerName }) => [
          ...
          receivable.dueDate ? new Date(receivable.dueDate) : "Non définie",
          receivable.originalAmount,
          receivable.balance,
          receivable.isSettled ? "Réglée" : "Ouverte",
        ])
      )
    );
  };
      ...
      <Card className="h-fit border-0 bg-[#fdf0dc]">
        <CardContent className="p-4 sm:p-6">
          <div className="flex items-center gap-2">
            <UserRoundPlus className="h-5 w-5 text-[#9d632d]" />
            <p className="font-serif text-xl">Nouveau client</p>
          </div>
          <p className="mt-2 text-sm text-[#7f7464]">
            Créez un client avant de lui accorder un crédit à la caisse.
          </p>
          <form
            onSubmit={event => {
          ...
          <div className="mt-7 border-t border-[#eddabf] pt-5">
            ...
            <div className="mt-3 space-y-2">
              {customers.data?.slice(0, 6).map(customer => (
                <p key={customer.id} className="break-words text-sm">
                  {customer.name}
                </p>
              ))}
            </div>
        ...
        <CardContent className="p-4 sm:p-7">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                <WalletCards className="h-5 w-5 text-[#a96551]" />
                <div>
                  <p className="font-serif text-xl">Créances</p>
                  <p className="mt-1 text-xs text-[#85877f]">
                    Filtrez et exportez les créances ouvertes ou réglées.
                  </p>
                </div>
              </div>
              <Button
                ...
                disabled={!receivables.data?.length}
              >
            ...
            <div className="flex flex-wrap items-center gap-3">
              <label className="grid min-w-40 gap-1 text-xs font-semibold text-[#687267]">
                Statut
                <select
                  value={status}
                  onChange={event => {
                    const next = event.target.value as
                      ...
                      | "all";
                    setStatus(next);
                    if (next === "settled") setOverdueOnly(false);
                  }}
              ...
              <label className="flex min-h-11 items-center gap-2 pt-5 text-sm font-medium text-[#687267]">
                <input
                  type="checkbox"
                  checked={overdueOnly}
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="break-words font-semibold">{customerName}</p>
                    <p className="mt-1 text-xs text-[#85877f]">
                      Solde initial :{" "}
                  ...
                  <p
                    className={
                      receivable.isSettled
                        ...
                        : "font-semibold text-[#b05d4c]"
                    }
                  >
                    {receivable.isSettled
                      ? "Réglée"
                      : format(receivable.balance, currency)}
                  </p>
                </div>
                {!receivable.isSettled && (
                  <div className="mt-4 grid gap-2 sm:flex sm:items-center sm:justify-end">
                    <Label htmlFor={`repayment-${receivable.id}`} className="sr-only">
                      Montant à encaisser pour {customerName}
                    </Label>
                    <Input
                      id={`repayment-${receivable.id}`}
                      type="number"
                      min="1"
                      max={receivable.balance}
                      value={repayments[receivable.id] ?? ""}
                      onChange={event =>
                        setRepayments(values => ({
                          ...values,
                    ...
                    <Button
                      className="w-full sm:w-auto"
                      disabled={repay.isPending || !repayments[receivable.id]}
                      onClick={() =>
                        recordRepayment(receivable.id, receivable.balance)
                      }
                    >
                      Encaisser
                    </Button>
