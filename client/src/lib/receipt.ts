...
export async function createReceiptPdf(receipt: SaleReceipt) {
  ...
  if (logo) {
    try {
      ...
      /* Logo facultatif : le reçu reste générable. */
    }
  }
...
export function buildReceiptHtml(receipt: SaleReceipt) {
  ...
  const logo = isShopLogoUrl(receipt.logoUrl)
    ...
    : "";
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>Reçu ${escapeHtml(receipt.saleNumber)}</title><style>body{font-family:Arial,sans-serif;margin:0;color:#1f2924}main{max-width:360px;margin:0 auto;padding:24px}.brand{display:flex;align-items:center;gap:12px;background:#26352d;color:#f5f7e8;padding:20px;border-radius:12px}.brand-logo{width:42px;height:42px;object-fit:contain;background:#fff;border-radius:8px;padding:3px}.brand h1{font-size:20px;margin:0}.brand small{display:block;margin-top:5px;color:#d8ef73;font-size:10px;font-weight:700;letter-spacing:.11em}p{font-size:12px;margin:6px 0;color:#526058}.number{font-size:12px;font-weight:700;margin-top:14px;color:#3d5839}.notice{background:#fff4cf;color:#6b5615;padding:8px;border-radius:6px;font-size:11px}table{width:100%;border-collapse:collapse;margin:18px 0;font-size:12px}td{padding:8px 0;border-bottom:1px solid #e6e8e3}td:last-child{text-align:right;font-weight:600}.summary{font-size:12px}.summary div{display:flex;justify-content:space-between;padding:4px 0}.total{font-size:16px!important;font-weight:700;border-top:1px solid #1f2924;margin-top:6px;padding-top:8px!important;color:#1f2924}.footer{text-align:center;margin-top:24px;font-size:11px;color:#67706b}@media print{body{print-color-adjust:exact}main{padding:0}.brand{border-radius:0}}</style></head><body><main><header class="brand">${logo}<div><h1>${escapeHtml(receipt.shopName)}</h1><small>REÇU DE VENTE</small></div></header><p class="number">${escapeHtml(receipt.saleNumber)}</p><p>${receipt.soldAt.toLocaleString("fr-FR", { dateStyle: "medium", timeStyle: "short" })}${receipt.customerName ? ` · ${escapeHtml(receipt.customerName)}` : ""}</p>${receipt.pendingSync ? '<p class="notice">Vente enregistrée hors ligne — numéro définitif après synchronisation.</p>' : ""}<table><tbody>${lines}</tbody></table><div class="summary"><div><span>Sous-total</span><span>${money(receipt.subtotal, receipt.currency)}</span></div>${receipt.discount > 0 ? `<div><span>Remise</span><span>− ${money(receipt.discount, receipt.currency)}</span></div>` : ""}<div class="total"><span>Total</span><span>${money(receipt.total, receipt.currency)}</span></div>${receipt.cash > 0 ? `<div><span>Espèces</span><span>${money(receipt.cash, receipt.currency)}</span></div>` : ""}${receipt.mobileMoney > 0 ? `<div><span>Mobile money</span><span>${money(receipt.mobileMoney, receipt.currency)}</span></div>` : ""}${receipt.credit > 0 ? `<div><span>À crédit</span><span>${money(receipt.credit, receipt.currency)}</span></div>` : ""}</div><p class="footer">Merci pour votre achat.</p></main></body></html>`;
}
