...
| Secret JWT permissif | La production exige désormais un `JWT_SECRET` configuré dès le démarrage Express et au chargement de la fonction tRPC. Le secret de développement ne reste disponible qu’en mode explicitement non production. | Tests de refus de secret absent et d’acceptation d’un secret configuré réussis. |
