...
describe("EASYSTOR schema on Neon", () => {
  it("contains the protected operational tables", async () => {
    ...
    const rows =
      await sql`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('users', 'shops', 'shop_members', 'products', 'sales', 'sale_items', 'receivables', 'repayments', 'stock_movements', 'sync_operations')`;
