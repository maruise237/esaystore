...
import {
  isPublicCatalogStorageKey,
  servePublicCatalogImage,
} from "./publicCatalogStorage";
...
describe("public catalog storage keys", () => {
  it("accepts only hashed catalogue image keys and dedicated shop logos", () => {
    expect(isPublicCatalogStorageKey(validKey)).toBe(true);
    expect(
      isPublicCatalogStorageKey(
        `shops/${shopId}/catalog/variant/${targetId}_a1b2c3d4.jpg`
      )
    ).toBe(true);
    expect(
  ...
  it("rejects traversal attempts and non-catalogue storage keys", async () => {
    ...
    expect(
      isPublicCatalogStorageKey(`shops/${shopId}/private/document.pdf`)
    ).toBe(false);
    expect(
      isPublicCatalogStorageKey(
