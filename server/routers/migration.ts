...
type Conflict = { type: "product" | "customer" | "sale" | "business_day" | "reimport"; sourceId: string; reason: string };
...
async function detectConflicts(shopId: string, data: Payload, fileHash: string): Promise<Conflict[]> {
  ...
  const salesByNumber = new Set(knownSales.map((row) => row.saleNumber)); const closed = new Set(closedDays.map((row) => row.businessDate));
  const seenProducts = new Set<string>(); const seenCustomers = new Set<string>(); const conflicts: Conflict[] = [];
...
export const migrationRouter = router({
  ...
  run: protectedProcedure.input(z.object({ shopId: z.string().uuid(), fileName: z.string().trim().min(1).max(240), data: payload, conflictStrategy })).mutation(async ({ ctx, input }) => {
    ...
    let productCount = 0; let customerCount = 0; let saleCount = 0; let saleItemCount = 0; let expenseCount = 0;
