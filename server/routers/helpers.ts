...
import { cashClosures, shops, type shopRoleEnum } from "../../drizzle/schema";
...
export async function assertShopAccess(
  ...
  if (!shop?.isActive) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message:
        "Cette boutique est temporairement suspendue. Contactez le support EASYSTOR.",
    });
  }
...
export async function assertBusinessDayOpen(
  ...
  const [closure] = await getDb()
    .select({ id: cashClosures.id })
    .from(cashClosures)
    .where(
      and(
        eq(cashClosures.shopId, shopId),
        eq(cashClosures.businessDate, businessDate)
      )
    )
    .limit(1);
  if (closure)
    throw new TRPCError({
      code: "CONFLICT",
      message: `La caisse du ${businessDate} est déjà clôturée. Enregistrez l’opération sur une nouvelle journée.`,
    });
}
