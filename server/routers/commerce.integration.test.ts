...
describe("commerce transaction with Neon", () => {
  ...
  it("aggregates daily and weekly performance with expenses without crossing shop boundaries", async () => {
    ...
    await caller().sales.checkout({ shopId, operationId: crypto.randomUUID(), soldAt: firstDay, discountAmount: 0, payment: { cash: 1_000, mobileMoney: 0 }, items: [{ productId, quantity: 1 }] });
    await caller().sales.checkout({ shopId, operationId: crypto.randomUUID(), soldAt: secondDay, discountAmount: 0, payment: { cash: 1_000, mobileMoney: 0 }, items: [{ productId, quantity: 1 }] });
    ...
    const from = new Date("2026-08-10T00:00:00.000Z"); const to = new Date("2026-08-11T23:59:59.999Z");
    const daily = await insightsCaller().report({ shopId, from, to, granularity: "day" });
    expect(daily.turnover).toBe(2_000); expect(daily.grossMargin).toBe(1_000); expect(daily.expenses).toBe(300); expect(daily.operatingResult).toBe(700); expect(daily.saleCount).toBe(2); expect(daily.timeline).toEqual(expect.arrayContaining([expect.objectContaining({ turnover: 1_000, expenses: 0, saleCount: 1 }), expect.objectContaining({ turnover: 1_000, expenses: 300, saleCount: 1 })]));
    const weekly = await insightsCaller().report({ shopId, from, to: new Date("2026-08-16T23:59:59.999Z"), granularity: "week" });
    expect(weekly.timeline).toHaveLength(1); expect(weekly.timeline[0]).toMatchObject({ turnover: 2_000, expenses: 300, saleCount: 2 });
  });
}, 60_000);
