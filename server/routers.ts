...
export const appRouter = router({
  ...
  shops: router({
    ...
    memberRole: protectedProcedure
      ...
      .query(({ ctx, input }) => assertShopAccess(ctx.user.id, input.shopId)),
    members: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        return getDb()
          ...
          .where(eq(shopMembers.shopId, input.shopId));
      }),
    addMember: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!member)
          throw new Error(
            "Ce collaborateur doit créer son compte EASYSTOR avant d’être ajouté."
          );
        await getDb()
          .insert(shopMembers)
          .values({ shopId: input.shopId, userId: member.id, role: input.role })
          .onConflictDoUpdate({
